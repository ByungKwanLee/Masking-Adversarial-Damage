...
def fetch_mat_weights(layer, use_patch=False):
    ...
    if isinstance(layer, nn.Conv2d) or isinstance(layer, Conv2d_mask):
        if use_patch:
    ...
    elif isinstance(layer, nn.Linear) or isinstance(layer, Linear_mask):
        weight = layer.weight
        if layer.bias is not None:
            weight = torch.cat([weight, layer.bias.unsqueeze(1)], 1)
    else:
...
def fetch_mat_mask_weights(layer, use_patch=False):
    # -> output_dium * input_dim (kh*kw*in_c + [1 if with bias])
    if isinstance(layer, nn.Conv2d) or isinstance(layer, Conv2d_mask):
        if use_patch:
            weight = layer.mask_weight.transpose(1, 2).transpose(2, 3)  # n_out * kh * kw * inc
            n_out, k_h, k_w, in_c = weight.size()
            weight = try_contiguous(weight)
            weight = weight.view(-1, weight.size(-1))
            bias = 0
            if layer.mask_bias is not None:
                copied_bias = torch.cat([layer.mask_bias.unsqueeze(1) for _ in range(k_h*k_w)], 1).view(-1, 1)
                weight = torch.cat([weight, copied_bias], 1)  # layer.mask_bias.unsqueeze(1)], 1)
                bias = 1
            weight = weight.view(n_out, k_h*k_w, in_c+bias)
        else:
            weight = layer.mask_weight  # n_filters * in_c * kh * kw
            # weight = weight.transpose(1, 2).transpose(2, 3).contiguous()
            weight = weight.view(weight.size(0), -1)
            if layer.mask_bias is not None:
                weight = torch.cat([weight, layer.mask_bias.unsqueeze(1)], 1)
    elif isinstance(layer, nn.Linear) or isinstance(layer, Linear_mask):
        weight = layer.mask_weight
        if layer.bias is not None:
            weight = torch.cat([weight, layer.mask_bias.unsqueeze(1)], 1)
    else:
        raise NotImplementedError
